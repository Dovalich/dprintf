...
/*   Updated: 2022/01/15 19:50:12 by noufel           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	format_parser(va_list	*ap, const char	*str, t_var_func	*ptr, int fd)
{
	int			i;
	...
	while (str[i])
	{
		if (str[i] == '%' && is_valid_argument(str + i + 1))
		{
			...
			nb_printed_chars += cur_elem->treat_function(ap, str + i, fd);
		}
		else
		{
			write(1, &str[i], 1);
...
int	treat_percent(va_list *varlist, const char *str, int fd)
{
...
int	ft_dprintf(int fd, const	char	*str, ...)
{
	...
	printed_chars = format_parser(&args, str, ptr, fd);
	va_end(args);
	free_list(ptr);
